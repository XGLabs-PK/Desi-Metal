...
public class FOVGizmoEditor : Editor
{
    void OnSceneGUI()
    {
        ...
        if (!enemy.canSeePlayers) return;
        Handles.color = Color.green;
        Handles.DrawLine(enemy.transform.position, enemy.player.position);
    }

    static Vector3 DirectionOfAngle(float eulerY, float angleDegrees)
    {
        angleDegrees += eulerY;

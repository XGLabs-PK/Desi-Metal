...
namespace EnhancedHierarchy
{
    ...
    {
        static MiniLabelProvider[] MiniLabelProviders => Preferences.miniLabelProviders;
        ...
        static void OnItemGUI(int id, Rect rect)
        {
            ...
            using (ProfilerSample.Get("Enhanced Hierarchy"))
            {
                try
                {
                    if (IsGameObject)
                    {
                        for (int i = 0; i < Preferences.RightIcons.Value.Count; i++)
                            Preferences.RightIcons.Value[i].SafeInit();

                        for (int i = 0; i < Preferences.LeftIcons.Value.Count; i++)
                            Preferences.LeftIcons.Value[i].SafeInit();

                        Preferences.LeftSideButton.SafeInit();

                        for (int i = 0; i < MiniLabelProviders.Length; i++)
                    ...
                    DrawTree(RawRect);
                    ChildToggle();
                    float trailingWidth = DoTrailing();
                    DrawHover();
                    ColorSort(RawRect);
                    ...
                    if (IsGameObject)
                    {
                        ...
                        for (int i = 0; i < Preferences.RightIcons.Value.Count; i++)
                            ...
                            {
                                IconBase icon = Preferences.RightIcons.Value[i];
                                rect.xMin -= icon.SafeGetWidth();
                                icon.SafeDoGUI(rect);
                                rect.xMax -= icon.SafeGetWidth();
                            }

                        Rect leftSideRect = RawRect;

                        if (Preferences.LeftmostButton)
                            leftSideRect.xMin = 0f;
                        else
                            leftSideRect.xMin -=
                                2f + CurrentGameObject.transform.childCount > 0 ||
                                Preferences.TreeOpacity > ALPHA_THRESHOLD
                                    ? 30f
                                    : 18f;
        ...
        static void DrawHover()
        {
            ...
            Color tint = Preferences.HoverTintColor.Value;
        ...
        static void IgnoreLockedSelection()
        {
            ...
            using (ProfilerSample.Get())
            {
                ...
                for (int i = 0; i < selection.Length; i++)
                    if (selection[i] is GameObject && (selection[i].hideFlags & HideFlags.NotEditable) != 0 &&
                        !EditorUtility.IsPersistent(selection[i]))
                    {
                        selection[i] = null;
                        changed = true;
                    }
        ...
        static void ChildToggle()
        {
            using (ProfilerSample.Get())
            {
                if (!Preferences.NumericChildExpand || !IsRepaintEvent || !IsGameObject ||
                    CurrentGameObject.transform.childCount <= 0)
                    return;

                Rect rect = RawRect;
                string childString = CurrentGameObject.transform.childCount.ToString("00");
                bool expanded = Reflected.GetTransformIsExpanded(CurrentGameObject);
                ...
                {
                    Styles.newToggleStyle.Draw(rect, Utility.GetTempGUIContent(childString), false, false, expanded,
                        false);
                }
            }
        }

        static void DrawHorizontalSeparator(Rect rect)
        {
            ...
            using (ProfilerSample.Get())
            {
                ...
                float height = Reflected.HierarchyWindowInstance.position.height;
                float count = (height - FinalRect.y) / FinalRect.height;
        ...
        static void ColorSort(Rect rect)
        {
            if (!IsRepaintEvent)
                return;
            ...
            using (ProfilerSample.Get())
            {
                ...
                Color rowTint = GetRowTint();
                LayerColor rowCustomTint = GetRowCustomTint();
                    ...
                    using (new GUIColor(rowCustomTint.color))
                    {
                        switch (rowCustomTint.mode)
                        {
                            ...
                            case TintMode.GradientLeftToRight:
                                GUI.DrawTexture(Utility.FlipRectHorizontally(rect), Styles.fadeTexture,
        ...
        static void DrawTree(Rect rect)
        {
            ...
            using (new GUIColor(Utility.GetHierarchyColor(CurrentGameObject.transform.parent), Preferences.TreeOpacity))
            {
                ...
                float indent = Reflected.HierarchyArea.Supported ? Reflected.HierarchyArea.IndentWidth : 16f;
                ...
                {
                    bool lastInHierarchy = Utility.TransformIsLastChild(CurrentGameObject.transform);
                    ...
                    */
                    ...
                    float extendStemProportion = CurrentGameObject.transform.childCount == 0
                        ? Preferences.TreeStemProportion.Value * indent
                        : indent - 14f;

                    if (extendStemProportion > 0.01f)
                    {
                        Rect extendedStemRect = new Rect(rect.x + rect.size.x, rect.y + (lastInHierarchy ? 9f : 8f),
                            extendStemProportion, 1f);
                ...
                Transform currentTransform = CurrentGameObject.transform.parent;
                ...
                for (rect.x -= indent; rect.xMin > 0f && currentTransform && currentTransform.parent; rect.x -= indent)
                {
                    if (!Utility.TransformIsLastChild(currentTransform))
                        using (new GUIColor(Utility.GetHierarchyColor(currentTransform.parent),
                                   Preferences.TreeOpacity))
                        {
        ...
        static void CalculateIconsWidth()
        {
            using (ProfilerSample.Get())
            {
                LeftIconsWidth = 0f;
        ...
        static void DrawLeftSideIcons(Rect rect)
        {
            if (!IsGameObject)
                return;
            ...
            using (ProfilerSample.Get())
            {
                ...
                rect.xMin = Math.Min(
                    rect.xMax - RightIconsWidth - LeftIconsWidth - CalcMiniLabelSize() - 5f - Preferences.RightMargin,
                    rect.xMin);

                for (int i = 0; i < Preferences.LeftIcons.Value.Count; i++)
                    using (new GUIBackgroundColor(Styles.backgroundColorEnabled))
                    {
                        IconBase icon = Preferences.LeftIcons.Value[i];
        ...
        static float DoTrailing()
        {
            ...
            using (ProfilerSample.Get())
            {
                float size = LabelSize; // CurrentStyle.CalcSize(Utility.GetTempGUIContent(GameObjectName)).x;
                float iconsWidth = RightIconsWidth + LeftIconsWidth + CalcMiniLabelSize() + Preferences.RightMargin;

                float iconsMin = FullSizeRect.xMax - iconsWidth;
                float labelMax = LabelOnlyRect.xMax;

                bool overlapping = iconsMin <= labelMax;

                if (!overlapping)
                    return labelMax;

                Rect rect = FullSizeRect;
                ...
                if (Selection.gameObjects.Contains(CurrentGameObject))
                    EditorGUI.DrawRect(rect,
                        Reflected.HierarchyFocused ? Styles.selectedFocusedColor : Styles.selectedUnfocusedColor);
                else
        ...
        static void DrawMiniLabel(ref Rect rect)
        {
            if (!IsGameObject)
                return;
            ...
            using (ProfilerSample.Get())
            {
                switch (MiniLabelProviders.Length)
                {
                    ...
                    default:
                        MiniLabelProvider ml0 = MiniLabelProviders[0];
                        MiniLabelProvider ml1 = MiniLabelProviders[1];
                        bool ml0HasValue = ml0.HasValue();
                        bool ml1HasValue = ml1.HasValue();
        ...
        static float CalcMiniLabelSize()
        {
        ...
        static void DrawTooltip(Rect rect, float fullTrailingWidth)
        {
            ...
            using (ProfilerSample.Get())
            {
                ...
                if (GameObjectUtility.GetStaticEditorFlags(CurrentGameObject) != 0)
                    tooltip.AppendFormat("\nStatic: {0}",
                        Utility.EnumFlagsToString(GameObjectUtility.GetStaticEditorFlags(CurrentGameObject)));
        ...
        static void DoSelection(Rect rect)
        {

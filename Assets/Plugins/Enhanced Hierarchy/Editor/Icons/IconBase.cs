...
namespace EnhancedHierarchy
{
    ...
    public abstract class IconBase
    {
        ...
        public virtual IconPosition Side => IconPosition.SafeArea;
        ...
        static IconBase()
        {
            ...
            AllLeftIcons = icons.Select(i => i.Value).Where(i => (i.Side & IconPosition.AfterObjectName) != 0)
                .ToArray();

            AllRightIcons = icons.Select(i => i.Value).Where(i => (i.Side & IconPosition.RightMost) != 0).ToArray();

            AllLeftOfNameIcons = icons.Select(i => i.Value).Where(i => (i.Side & IconPosition.BeforeObjectName) != 0)
                .ToArray();
        }
        ...
        static readonly Dictionary<string, IconBase> icons = new Dictionary<string, IconBase>();
                ...
                default:
                    for (int i = 0; i < objs.Count; i++)
                        if (objs[i] && objs[i].transform.childCount > 0)
                            try
                            {
                                return (ChildrenChangeMode)EditorUtility.DisplayDialogComplex(title, message,
        ...
        protected static List<GameObject> GetSelectedObjectsAndCurrent()
        {
            ...
            return Selection.gameObjects
                .Where(obj =>
                    !EditorUtility.IsPersistent(obj)) // Makes sure the object is part of the scene and not the project

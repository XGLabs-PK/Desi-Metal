...
namespace EnhancedHierarchy
{
    public static partial class Preferences
    {
        ...
        static readonly GUIContent resetSettingsContent =
            new GUIContent("Use Defaults", "Reset all settings to default");
        static readonly GUIContent unlockAllContent = new GUIContent("Unlock All Objects",
        ...
        static readonly GUIContent mailDeveloperContent =
            new GUIContent("Support Email", "Request support from the developer\n\n" + DEVELOPER_EMAIL);
        static readonly GUIContent versionContent =
            new GUIContent(string.Format("Version: {0} ({1:d})", pluginVersion, pluginDate));

        static Vector2 scroll;

        static ReorderableList leftIconsList, rightIconsList, rowColorsList;

        static readonly string[] minilabelsNames;

        static GenericMenu LeftIconsMenu => GetGenericMenuForIcons(LeftIcons, IconBase.AllLeftIcons);
        static GenericMenu RightIconsMenu => GetGenericMenuForIcons(RightIcons, IconBase.AllRightIcons);

        static GenericMenu RowColorsMenu
        {
            get
            {
                ...
                for (int i = 0; i < 32; i++)
                {
                    ...
                    TintMode mode = PerLayerRowColors.Value.LastOrDefault().mode;
                    string layerName = LayerMask.LayerToName(i);
                    LayerColor layer = new LayerColor(i, randomColor, mode);
        ...
        static GenericMenu GetGenericMenuForIcons<T>(PrefItem<T> preferenceItem, IconBase[] icons) where T : IList
        {
            GenericMenu menu = new GenericMenu();

            foreach (IconBase i in icons)
            {
                IconBase icon = i;
        ...
        static ReorderableList GenerateReordableList<T>(PrefItem<T> preferenceItem) where T : IList
        {
            ReorderableList result = new ReorderableList(preferenceItem.Value, typeof(T), true, true, true, true);
            ...
            {
                IconBase icon = result.list[index] as IconBase;
                ...
                GUIContent content =
                    Utility.GetTempGUIContent(icon.Name, icon.PreferencesTooltip, icon.PreferencesPreview);

                bool whiteTexture = content.image ? content.image.name.Contains("eh_icon_white") : true;
        ...
        [SettingsProvider]
        static SettingsProvider RetrieveSettingsProvider()
        {
            SettingsProvider settingsProvider = new SettingsProvider("Preferences/Enhanced Hierarchy",
        ...
        static void OnPreferencesGUI(string search)
        {
            ...
            EditorGUILayout.HelpBox("Each item has a tooltip explaining what it does, keep the mouse over it to see.",
                MessageType.Info);
            ...
            using (Enabled.GetEnabledScope())
            {
                ...
                using (new GUIIndent("Row separators"))
                {
                    ...
                    GUI.changed = false;
                    Rect rect = EditorGUILayout.GetControlRect(false, rowColorsList.GetHeight());
                    rect.xMin += EditorGUI.indentLevel * 16f;
                ...
                using (new GUIIndent("Children behaviour on change"))
                {
                    ...
                    EditorGUILayout.HelpBox(
                        string.Format(
                            "Pressing down {0} while clicking on a button will make it temporary have the opposite children change mode",
                            Utility.CtrlKey), MessageType.Info);
                }
        ...
        static string GetEmailURL(Exception e)
        {
            StringBuilder full = new StringBuilder();
            StringBuilder body = new StringBuilder();

#if UNITY_2018_1_OR_NEWER
            Func<string, string> EscapeURL = url =>
            {
        ...
        static LayerColor LayerColorField(Rect rect, LayerColor layerColor)
        {
            LayerColor value = layerColor;
            Rect rect1 = rect;
            Rect rect2 = rect;
            Rect rect3 = rect;
            Rect rect4 = rect;

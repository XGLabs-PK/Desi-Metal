...
namespace AllIn1VfxToolkit.Demo.Scripts
{
    ...
    {
        [SerializeField]
        int startingCollectionIndex, startingEffectIndex;

        [Space]
        [Header("Demo Effects")]
        [SerializeField]
        All1VfxDemoEffectCollection[] effectsToSpawnCollections;

        [Space]
        [Header("Projectile References")]
        [Space]
        [SerializeField]
        GameObject projectileBasePrefab;
        [SerializeField]
        GameObject projectileSceneSetupObject;
        [SerializeField]
        ...
        [SerializeField]
        KeyCode playEffectKey = KeyCode.Q;
        [SerializeField]
        KeyCode nextEffectKey = KeyCode.RightArrow;
        [SerializeField]
        KeyCode nextEffectKeyAlt = KeyCode.D;
        [SerializeField]
        KeyCode previousEffectKey = KeyCode.LeftArrow;
        [SerializeField]
        KeyCode previousEffectKeyAlt = KeyCode.A;

        [Space]
        [Header("UI and Other References")]
        [SerializeField]
        Text currentEffectLabel;
        [SerializeField]
        Button playEffectButton;
        [SerializeField]
        GameObject playEffectInstructionsGo;
        [SerializeField]
        Button nextEffectButton, previousEffectButton;
        [SerializeField]
        Transform groundSpawnTransform, cameraPivotTransform;
        [SerializeField]
        float camPivotHeightSmoothing;
        [SerializeField]
        GameObject projectileEffectUI, normalEffectUI;


        All1VfxDemoEffect currDemoEffect;
        int currDemoCollectionIndex, currDemoEffectIndex, currentEffectPlays;
        AllIn1DemoScaleTween currLabelTween, playButtTween, nextButtTween, prevButtTween;
        float timeSinceEffectPlay;
        AllIn1TimeControl allIn1TimeControl;
        ...
        void Update()
        {
            ...
            if (currDemoEffect.spawnTouchingFloor)
                cameraPivotTransform.position = Vector3.Lerp(cameraPivotTransform.position, new Vector3(0f, 0.1f, 0f),
                    Time.unscaledDeltaTime * camPivotHeightSmoothing);

            if (!currDemoEffect.spawnTouchingFloor)
                cameraPivotTransform.position = Vector3.Lerp(cameraPivotTransform.position, new Vector3(0f, 2f, 0f),
                    Time.unscaledDeltaTime * camPivotHeightSmoothing);
        ...
        public void PlayCurrentEffect(bool isAfterSetupAndInstantiateEffect = false)
        {
            if (currentEffectPlays > 0 && timeSinceEffectPlay < currDemoEffect.cooldown)
                return; //Return if on cooldown and not first play
            ...
            if (currDemoEffect.isShootProjectile)
            {
                ...
                {
                    tempTransform = Instantiate(currDemoEffect.muzzleFlashPrefab, projectileSpawnPoint.position,
                ...
                Transform projectileBase =
                    Instantiate(projectileBasePrefab, projectileSpawnPoint.position, Quaternion.identity).transform;
                ...
                tempTransform = Instantiate(currDemoEffect.projectilePrefab, projectileSpawnPoint.position,
                    Quaternion.identity).transform;
                ...
                tempProjectileInstance.Initialize(transform, projectileSpawnPoint.forward,
                    currDemoEffect.projectileSpeed, currDemoEffect.impactPrefab, currDemoEffect.scaleMultiplier);

                if (currDemoEffect.doCameraShake)
                    tempProjectileInstance.AddScreenShakeOnImpact(currDemoEffect.projectileImpactShakeAmount);
            }
            else
            {
                ...
                if (currDemoEffect.canBePlayedAgain && currDemoEffect.randomSpreadRadius > 0f && currentEffectPlays > 0)
                    tempTransform.position += new Vector3(
                        Random.Range(-currDemoEffect.randomSpreadRadius, currDemoEffect.randomSpreadRadius), 0f,
        ...
        void SetupAndInstantiateCurrentEffect()
        {
            DestroyAllChildren();
            currentEffectPlays = 0;
            ComputeValidEffectAndCollectionIndex();

            currDemoEffect = effectsToSpawnCollections[currDemoCollectionIndex]
                .demoEffectCollection[currDemoEffectIndex];
            ...
            currentEffectLabel.text = currDemoEffect.isShootProjectile
                ? currDemoEffect.projectilePrefab.name
                : currDemoEffect.effectPrefab.name;
        ...
        void ComputeValidEffectAndCollectionIndex()
        {
            int demoEffectOperation =
                0; // 0 means no operation, 1 means assign first collection effect, 2 means last collection effect
            ...
            else if (currDemoEffectIndex >=
                     effectsToSpawnCollections[currDemoCollectionIndex].demoEffectCollection.Length)
            {
                currDemoCollectionIndex++;
            ...
            if (demoEffectOperation > 0)
            {
                ...
                else if (demoEffectOperation == 2)
                    currDemoEffectIndex =
                        effectsToSpawnCollections[currDemoCollectionIndex].demoEffectCollection.Length - 1;
            }
        }

        IEnumerator CurrentEffectLabelTweenEffectCR()
        {

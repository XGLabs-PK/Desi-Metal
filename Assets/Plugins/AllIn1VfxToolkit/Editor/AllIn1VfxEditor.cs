...
{
    [CustomEditor(typeof(AllIn1VfxComponent))]
    [CanEditMultipleObjects]
    public class AllIn1VfxEditor : Editor
    {
        GUIStyle smallBoldLabel;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("Deactivate All Effects"))
                for (int i = 0; i < targets.Length; i++)
                    ((AllIn1VfxComponent)targets[i]).ClearAllKeywords();

            if (GUILayout.Button("New Clean Material"))
                for (int i = 0; i < targets.Length; i++)
                    ((AllIn1VfxComponent)targets[i]).TryCreateNew();

            if (GUILayout.Button("Create New Material With Same Properties (SEE DOC)"))
                for (int i = 0; i < targets.Length; i++)
                    ((AllIn1VfxComponent)targets[i]).MakeCopy();

            if (GUILayout.Button("Save Material To Folder (SEE DOC)"))
                for (int i = 0; i < targets.Length; i++)
                    ((AllIn1VfxComponent)targets[i]).SaveMaterial();

            if (GUILayout.Button("Apply Material To All Children"))
                for (int i = 0; i < targets.Length; i++)
                    ((AllIn1VfxComponent)targets[i]).ApplyMaterialToHierarchy();

            if (GUILayout.Button("Render Material To Image"))
                for (int i = 0; i < targets.Length; i++)
                    ((AllIn1VfxComponent)targets[i]).RenderToImage();
        ...
        void CheckIfShowParticleSystemHelperUI()
        {
            if (Selection.activeGameObject?.GetComponent<ParticleSystem>() == null) return;

            AllIn1ParticleHelperComponent all1VfxPsHelper =
                Selection.activeGameObject.GetComponent<AllIn1ParticleHelperComponent>();
            ...
            if (GUILayout.Button("Add Particle System Helper"))
                for (int i = 0; i < targets.Length; i++)
                    ((AllIn1VfxComponent)targets[i]).AddHelperAndPlaceUnderAll1VfxMainComponent();
        }

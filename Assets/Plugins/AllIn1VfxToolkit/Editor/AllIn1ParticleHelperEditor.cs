...
{
    [CustomEditor(typeof(AllIn1ParticleHelperComponent))]
    [CanEditMultipleObjects]
    public class AllIn1ParticleHelperEditor : Editor
    {
        GUIStyle smallRedBoldLabel, smallBoldLabel, headerBoldLabel, propertiesStyle;

        AllIn1VfxComponent all1Vfx;
        ParticleSystem ps;
        ParticleSystem.MainModule mainModule;
        Color targetColor = Color.white;
        int gradientColorKeyIndex = 0;
        int helperNum;
        SerializedProperty startColorProperty;
        ...
        {
            smallRedBoldLabel = new GUIStyle(EditorStyles.boldLabel)
                { normal = { textColor = new Color(0.58f, 0f, 0f) } };

            smallBoldLabel = new GUIStyle(EditorStyles.boldLabel);

            headerBoldLabel = new GUIStyle(EditorStyles.boldLabel)
            ...
            if (ps == null)
            {
                ...
                if (GUILayout.Button("Add New Particle System"))
                    Selection.activeGameObject.AddComponent<ParticleSystem>();

                return;
            }

            if (Selection.activeGameObject != null)
                all1Vfx = Selection.activeGameObject.GetComponent<AllIn1VfxComponent>();
        ...
        void ColorChangeUI()
        {
            ...
            if (toggle)
            {
                ...
                {
                    EditorGUILayout.IntField("Gradient Reference Index", gradientColorKeyIndex,
        ...
        void GeneralOptions()
        {
            ...
            if (toggle)
            {
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    for (int i = 0; i < targets.Length; i++)
                        ((AllIn1ParticleHelperComponent)targets[i]).minLifetime = minLifetime;

                    for (int i = 0; i < targets.Length; i++)
                        ((AllIn1ParticleHelperComponent)targets[i]).maxLifetime = maxLifetime;
                }
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    for (int i = 0; i < targets.Length; i++)
                        ((AllIn1ParticleHelperComponent)targets[i]).minSpeed = minSpeed;

                    for (int i = 0; i < targets.Length; i++)
                        ((AllIn1ParticleHelperComponent)targets[i]).maxSpeed = maxSpeed;
                }
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    for (int i = 0; i < targets.Length; i++)
                        ((AllIn1ParticleHelperComponent)targets[i]).minSize = minSize;

                    for (int i = 0; i < targets.Length; i++)
                        ((AllIn1ParticleHelperComponent)targets[i]).maxSize = maxSize;
                }
        ...
        void EmissionOptions()
        {
            ...
            if (toggle)
            {
                ...
                EditorGUILayout.MinMaxSlider("Number of Particles", ref minNumberOfParticles, ref maxNumberOfParticles,
                    0, 100);

                minNumberOfParticles =
                    EditorGUILayout.IntField("Min Particles", Mathf.RoundToInt(minNumberOfParticles));

                maxNumberOfParticles =
                    EditorGUILayout.IntField("Max Particles", Mathf.RoundToInt(maxNumberOfParticles));
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    for (int i = 0; i < targets.Length; i++)
                        ((AllIn1ParticleHelperComponent)targets[i]).minNumberOfParticles =
                            Mathf.RoundToInt(minNumberOfParticles);

                    for (int i = 0; i < targets.Length; i++)
                        ((AllIn1ParticleHelperComponent)targets[i]).maxNumberOfParticles =
                            Mathf.RoundToInt(maxNumberOfParticles);
                }
        ...
        void ShapeOptions()
        {
            ...
            if (toggle)
            {
                ...
                AllIn1ParticleHelperComponent.EmissionShapes tempSettings =
                    ((AllIn1ParticleHelperComponent)target).currEmissionShape;

                EditorGUI.BeginChangeCheck();

                tempSettings =
                    (AllIn1ParticleHelperComponent.EmissionShapes)EditorGUILayout.EnumPopup("New Shape", tempSettings);
        ...
        void OverLifetimeOptions()
        {
            ...
            if (toggle)
            {
                ...
                AllIn1ParticleHelperComponent.LifetimeSettings tempColorLifetime =
                    ((AllIn1ParticleHelperComponent)target).colorLifetime;

                EditorGUI.BeginChangeCheck();

                tempColorLifetime =
                    (AllIn1ParticleHelperComponent.LifetimeSettings)EditorGUILayout.EnumPopup("Alpha Over Lifetime",
                        tempColorLifetime);
                ...
                AllIn1ParticleHelperComponent.LifetimeSettings tempScaleLifetime =
                    ((AllIn1ParticleHelperComponent)target).sizeLifetime;

                EditorGUI.BeginChangeCheck();

                tempScaleLifetime =
                    (AllIn1ParticleHelperComponent.LifetimeSettings)EditorGUILayout.EnumPopup("Scale Over Lifetime",
                        tempScaleLifetime);
        ...
        int selectedIndex = 0;
        AllIn1ParticleHelperSO[] particleHelpersPresets = null;

        void ParticleHelperPresets()
        {
            ...
            if (toggle)
            {
                ...
                {
                    for (int i = 0; i < targets.Length; i++)
                        ((AllIn1ParticleHelperComponent)targets[i]).SaveParticleHelperPreset();
                ...
                if (particleHelpersPresets == null)
                    particleHelpersPresets = Resources.LoadAll<AllIn1ParticleHelperSO>("");

                bool particleHelpersPresetsAvailable = particleHelpersPresets.Length > 0;

                if (!particleHelpersPresetsAvailable)
                    GUILayout.Label("No Particle Helper Presets found, \nthey should be placed in a Resource folder",
                        smallRedBoldLabel);
                else
                {
                    string[] presetNames = new string[particleHelpersPresets.Length];

                    for (int i = 0; i < particleHelpersPresets.Length; i++)
                        presetNames[i] = particleHelpersPresets[i].name;
                    ...
                    if (GUILayout.Button("Apply Particle Helper Preset"))
                        for (int i = 0; i < targets.Length; i++)
                            ((AllIn1ParticleHelperComponent)targets[i]).ApplyParticleHelperPreset(
                                particleHelpersPresets[selectedIndex], true);
                }
        ...
        Preset[] particleSystemPresets = null;

        void ParticleSystemPresets()
        {
            ...
            if (toggle)
            {
                ...
                {
                    for (int i = 0; i < targets.Length; i++)
                        ((AllIn1ParticleHelperComponent)targets[i]).SaveParticleSystemPreset();
                ...
                if (!particleSystemPresetsAvailable)
                    GUILayout.Label("No Particle System Presets found, \nthey should be placed in a Resource folder",
                        smallRedBoldLabel);
                else
                {
                    string[] presetNames = new string[particleSystemPresets.Length];

                    for (int i = 0; i < particleSystemPresets.Length; i++)
                        presetNames[i] = particleSystemPresets[i].name;
                    ...
                    if (GUILayout.Button("Apply Particle System Preset"))
                        for (int i = 0; i < targets.Length; i++)
                            ((AllIn1ParticleHelperComponent)targets[i]).ApplyParticleSystemPreset(
                                particleSystemPresets[selectedIndex]);
                }

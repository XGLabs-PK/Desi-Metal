...
public class AllIn1VfxCustomMaterialEditor : ShaderGUI
{
    ...
    CompareFunction zTestMode;
    CullMode cullMode;
    ColorWriteMask colorMask;

    Material originalMaterialCopy;
    MaterialEditor matEditor;
    MaterialProperty[] matProperties;
    uint[] materialDrawers = new uint[] { 1, 2, 4, 8, 16, 32 };
    bool[] currEnabledDrawers;
    const uint advancedConfigDrawer = 0;
    const uint advancedShapeBlendDrawer = 1;
    const uint mainShapeDrawer = 2;
    const uint colorFxShapeDrawer = 3;
    const uint alphaFxShapeDrawer = 4;
    const uint uvFxShapeDrawer = 5;

    enum RenderingPreset
    {
        Transparent,
        Additive,
        SoftAdditive,
    ...
    RenderingPreset renderPreset;

    string[] oldKeyWords;
    GUIStyle propertiesStyle,
        bigLabel,
    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
    {
        ...
        uint iniDrawers = (uint)FindProperty("_EditorDrawers", matProperties).floatValue;
        ...
        if (Selection.activeGameObject != null)
            isPs = Selection.activeGameObject.GetComponent<ParticleSystem>() != null;
        ...
        currEnabledDrawers[advancedConfigDrawer] = GUILayout.Toggle(currEnabledDrawers[advancedConfigDrawer],
            new GUIContent("<size=12>Show Advanced Configuration</size>"), toggleButtonStyle);
        ...
        currEnabledDrawers[mainShapeDrawer] = GUILayout.Toggle(currEnabledDrawers[mainShapeDrawer],
            new GUIContent("<size=12>Show Shape Configuration</size>"), toggleButtonStyle);
        ...
        currEnabledDrawers[colorFxShapeDrawer] = GUILayout.Toggle(currEnabledDrawers[colorFxShapeDrawer],
            new GUIContent("Show Color Effects"), toggleButtonStyle);
        ...
        currEnabledDrawers[alphaFxShapeDrawer] = GUILayout.Toggle(currEnabledDrawers[alphaFxShapeDrawer],
            new GUIContent("Show Alpha Effects"), toggleButtonStyle);
        ...
        if (currEnabledDrawers[alphaFxShapeDrawer])
        {
            ...
            GenericEffect("Alpha Cutoff", "ALPHACUTOFF_ON", 52, 52, true,
                new string[] { "Use this to clip transparent pixels and reduce overdraw" });
        }

        SectionSeparatorAndHeader("UV and Vertex Effects");

        currEnabledDrawers[uvFxShapeDrawer] = GUILayout.Toggle(currEnabledDrawers[uvFxShapeDrawer],
            new GUIContent("Show UV Effects"), toggleButtonStyle);
        ...
        if (currEnabledDrawers[uvFxShapeDrawer])
        {
            ...
            if (isPs || oldKeyWords.Contains("SHAPEWEIGHTS_ON"))
                ShapeWeightVertexStream("Shape Weights Custom Stream", "SHAPEWEIGHTS_ON", 156);

            if (isPs || oldKeyWords.Contains("OFFSETSTREAM_ON"))
                OffsetVertexStream("Texture Offset Custom Stream", "OFFSETSTREAM_ON", 100);
            ...
            GenericEffect("Pixelate", "PIXELATE_ON", 77, 77, true,
                new string[] { "Looks bad with distortion effects" });
    ...
    void SetAndSaveEnabledDrawers(uint iniDrawers)
    {
        uint currDrawers = 0;
        ...
        if (iniDrawers != currDrawers) FindProperty("_EditorDrawers", matProperties).floatValue = currDrawers;
    }

    void SectionSeparatorAndHeader(string headerText)
    {
        EditorGUILayout.Separator();
        DrawLine(Color.grey, 1, 3);
    ...
    void ShapeResult()
    {
        ...
        currEnabledDrawers[advancedShapeBlendDrawer] = GUILayout.Toggle(currEnabledDrawers[advancedShapeBlendDrawer],
            new GUIContent("Show Shape Combination Options"), toggleButtonStyle);
        ...
        if (currEnabledDrawers[advancedShapeBlendDrawer])
        {
            ...
            if (splitColor)
            {
                ...
                if (shape3On)
                {
                    GUILayout.Label("RGB = ((Sh1.rgb * " + matProperties[21].floatValue + ") " + operationChar +
                                    " (Sh2.rgb *  " + matProperties[33].floatValue
                                    + ")) " + operationChar + " (Sh3.rgb * " + matProperties[45].floatValue + ")",
                        smallBoldLabel, GUILayout.Height(15));

                    if (addResults)
                        GUILayout.Label("A = max(max(Sh1.a * " + matProperties[22].floatValue + " , " + "Sh2.a *  " +
                                        matProperties[34].floatValue
                                        + "), Sh3.a * " + matProperties[46].floatValue + ")", smallBoldLabel,
                            GUILayout.Height(15));
                    else
                        GUILayout.Label("A = ((Sh1.a * " + matProperties[22].floatValue + ") " + operationChar +
                                        " (Sh2.a *  " + matProperties[34].floatValue
                                        + ")) " + operationChar + " (Sh3.a * " + matProperties[46].floatValue + ")",
                            smallBoldLabel, GUILayout.Height(15));
                }
                else
                {
                    GUILayout.Label(
                        "RGB = (Sh1.rgb * " + matProperties[21].floatValue + ") " + operationChar + " (Sh2.rgb *  " +
                        matProperties[33].floatValue + ")", smallBoldLabel, GUILayout.Height(15));

                    if (addResults)
                        GUILayout.Label(
                            "A = max(Sh1.a * " + matProperties[22].floatValue + " , Sh2.a *  " +
                            matProperties[34].floatValue + ")", smallBoldLabel, GUILayout.Height(15));
                    else
                        GUILayout.Label(
                            "A = (Sh1.a * " + matProperties[22].floatValue + ") " + operationChar + " (Sh2.a *  " +
                            matProperties[34].floatValue + ")", smallBoldLabel, GUILayout.Height(15));
                }
            }
            else
            {
                ...
                {
                    if (!addResults)
                        GUILayout.Label("Result = ((Shape1 * " + matProperties[21].floatValue + ") " + operationChar +
                                        " (Shape2 *  " + matProperties[33].floatValue
                                        + ")) " + operationChar + " (Shape3 * " + matProperties[45].floatValue + ")",
                            smallBoldLabel, GUILayout.Height(15));
                    else
                    {
                        GUILayout.Label("RGB = ((Sh1.rgb * " + matProperties[21].floatValue + ") " + operationChar +
                ...
                else
                {
                    if (!addResults)
                        GUILayout.Label(
                            "Result = (Shape1 * " + matProperties[21].floatValue + ") " + operationChar +
                            " (Shape2 *  " + matProperties[33].floatValue + ")", smallBoldLabel, GUILayout.Height(15));
                    else
                    {
                        GUILayout.Label(
                            "RGB = (Sh1.rgb * " + matProperties[21].floatValue + ") " + operationChar +
                            " (Sh2.rgb *  " + matProperties[33].floatValue + ")", smallBoldLabel, GUILayout.Height(15));
    ...
    void RenderingMode()
    {
...

        {
            ...
            if (renderPreset == RenderingPreset.Custom)
                renderPreset = (RenderingPreset)GUILayout.SelectionGrid((int)renderPreset,
                    new string[]
                        { "Transparent", "Additive", "Soft Add", "Blend Add / Premultiply", "Opaque", "Custom" }, 3,
                    smallTextStyle);
            else
                renderPreset = (RenderingPreset)GUILayout.SelectionGrid((int)renderPreset,
                    new string[] { "Transparent", "Additive", "Soft Add", "Blend Add / Premultiply", "Opaque" }, 3,
                    smallTextStyle);
            ...
            if (tempValue != renderM.floatValue && !Application.isPlaying)
            {
                switch (renderPreset)
                {
                    case RenderingPreset.Transparent:
                        ...
                        targetMat.SetInt("_SrcMode", (int)BlendMode.SrcAlpha);
                        targetMat.SetInt("_DstMode", (int)BlendMode.OneMinusSrcAlpha);
                        if (targetMat.renderQueue < 2050) targetMat.renderQueue = (int)RenderQueue.Transparent;
                    ...
                    case RenderingPreset.Additive:
                        targetMat.SetOverrideTag("RenderType", "Transparent");
                        targetMat.SetInt("_SrcMode", (int)BlendMode.One);
                        targetMat.SetInt("_DstMode", (int)BlendMode.One);
                        if (targetMat.renderQueue < 2050) targetMat.renderQueue = (int)RenderQueue.Transparent;
                    ...
                    case RenderingPreset.SoftAdditive:
                        targetMat.SetOverrideTag("RenderType", "Transparent");
                        targetMat.SetInt("_SrcMode", (int)BlendMode.OneMinusDstColor);
                        targetMat.SetInt("_DstMode", (int)BlendMode.One);
                        if (targetMat.renderQueue < 2050) targetMat.renderQueue = (int)RenderQueue.Transparent;
                        targetMat.EnableKeyword("ADDITIVECONFIG_ON");
                        targetMat.DisableKeyword("PREMULTIPLYALPHA_ON");
                    ...
                    case RenderingPreset.Opaque:
                        ...
                        targetMat.SetInt("_SrcMode", (int)BlendMode.One);
                        targetMat.SetInt("_DstMode", (int)BlendMode.Zero);
                        if (targetMat.renderQueue >= 2050) targetMat.renderQueue = (int)RenderQueue.Geometry;
    ...
    void Blending()
    {
...

        {
            ...
            srcMode = (BlendMode)EditorGUILayout.EnumPopup("Src", srcMode);
            if (tempValue != (float)srcMode && !Application.isPlaying) SaveAndSetCustomConfig();
            EditorGUILayout.Space();
            tempValue = (float)dstMode;
            dstMode = (BlendMode)EditorGUILayout.EnumPopup("Dst", dstMode);
    ...
    void ZWrite()
    {
        MaterialProperty zWrite = FindProperty("_ZWrite", matProperties);
    ...
    void ZTest()
    {
        MaterialProperty zTestM = FindProperty("_ZTestMode", matProperties);
        tempValue = zTestM.floatValue;
        SetLabelAndFieldWidth(72, 75);
        zTestMode = (CompareFunction)zTestM.floatValue;
        zTestMode = (CompareFunction)EditorGUILayout.EnumPopup("Z TestMode", zTestMode);
    ...
    void Culling()
    {
        ...
        cullMode = (CullMode)cullO.floatValue;
        cullMode = (CullMode)EditorGUILayout.EnumPopup("Culling Mode", cullMode);
    ...
    void ColorMask()
    {
        ...
        colorMask = (ColorWriteMask)colorM.floatValue;
        colorMask = (ColorWriteMask)EditorGUILayout.EnumPopup("Color Write Mask", colorMask);
        colorM.floatValue = (float)colorMask;
        if (tempValue != colorM.floatValue && !Application.isPlaying) SaveAndSetCustomConfig();
    }

    void Fog(string inspector, string keyword)
    {
    ...
    void ShapeProperties(int shapeNumber, int firstProperty, bool onlyIfKeyword = false,
        bool getIfEnabledAndReturn = false)
    {
        ...
        if (!onlyIfKeyword || toggle)
        {
            ...
            GenericEffect("Shape " + shapeNumber + " Rotation", nextShapeKeyword, shapeRotationPropertyIndex,
                shapeRotationPropertyIndex + 1, false);

            nextShapeKeyword = shapeKeyword + "SHAPECOLOR_ON";

            GenericEffect("Shape " + shapeNumber + " RGB is Shape Color, Red Channel Is Alpha", nextShapeKeyword, -1,
                -1, false);
            ...
            GenericEffect("Shape " + shapeNumber + " Screen Position UVs?", nextShapeKeyword,
                shapeSSUvScalePropertyIndex, shapeSSUvScalePropertyIndex, false);

            nextShapeKeyword = "SHAPEDEBUG_ON";
            int shapeDebugPropertyIndex = matProperties.Length - 1;

            bool ini = oldKeyWords.Contains(nextShapeKeyword) &&
                       matProperties[shapeDebugPropertyIndex].floatValue < shapeNumber + 0.5f &&
                       matProperties[shapeDebugPropertyIndex].floatValue > shapeNumber - 0.5f;
    ...
    void CopyShapePropertiesAndKeywords(int shapeToFirstProperty, int shapeFromFirstProperty, int shapeToNumber,
        int shapeFromNumber)
    {
        ...
        if (oldKeyWords.Contains("SHAPE" + shapeFromNumber + "CONTRAST_ON"))
            targetMat.EnableKeyword("SHAPE" + shapeToNumber + "CONTRAST_ON");
        else targetMat.DisableKeyword("SHAPE" + shapeToNumber + "CONTRAST_ON");

        if (oldKeyWords.Contains("SHAPE" + shapeFromNumber + "DISTORT_ON"))
            targetMat.EnableKeyword("SHAPE" + shapeToNumber + "DISTORT_ON");
        else targetMat.DisableKeyword("SHAPE" + shapeToNumber + "DISTORT_ON");

        if (oldKeyWords.Contains("SHAPE" + shapeFromNumber + "ROTATE_ON"))
            targetMat.EnableKeyword("SHAPE" + shapeToNumber + "ROTATE_ON");
        ...
        if (oldKeyWords.Contains("SHAPE" + shapeFromNumber + "SHAPECOLOR_ON"))
            targetMat.EnableKeyword("SHAPE" + shapeToNumber + "SHAPECOLOR_ON");
        else targetMat.DisableKeyword("SHAPE" + shapeToNumber + "SHAPECOLOR_ON");

        if (oldKeyWords.Contains("SHAPE" + shapeFromNumber + "SCREENUV_ON"))
            targetMat.EnableKeyword("SHAPE" + shapeToNumber + "SCREENUV_ON");
        else targetMat.DisableKeyword("SHAPE" + shapeToNumber + "SCREENUV_ON");

        SetShaderBasedOnEffectsAndPipeline();
    }

    void CopyShapeProperty(int propertyTo, int propertyFrom)
    {
        if (matProperties[propertyTo].type == MaterialProperty.PropType.Float ||
            matProperties[propertyTo].type == MaterialProperty.PropType.Range)
    ...
    void GenericEffect(string inspector, string keyword, int first, int last, bool effectCounter = true,
    ...
    void Glow(string inspector, string keyword)
    {
    ...
    void ColorRamp(string inspector, string keyword, string keyword2)
    {
        if (SpecialCaseEffectBodyToggle(inspector, keyword))
        {
            EditorGUILayout.BeginVertical(propertiesStyle);
    ...
    void AlphaSmoothstep(string inspector, string keyword, int first, int last)
    {
        if (SpecialCaseEffectBodyToggle(inspector, keyword))
        {
            EditorGUILayout.BeginVertical(propertiesStyle);
    ...
    void CameraDistFade(string inspector, string keyword, int first, int last)
    {
        if (SpecialCaseEffectBodyToggle(inspector, keyword))
        {
            EditorGUILayout.BeginVertical(propertiesStyle);
    ...
    void AlphaFade(string inspector, string keyword, int first, int last)
    {
        if (SpecialCaseEffectBodyToggle(inspector, keyword))
        {
            EditorGUILayout.BeginVertical(propertiesStyle);

            {
                ...
                if (isPs)
                    if (DrawEffectSubKeywordToggle("Fade Amount Driven By Vertex Stream? Custom1.y (See Docs)",
                            "ALPHAFADEINPUTSTREAM_ON"))
                        GUILayout.Label(
                            "*Custom Data Auto Setup button on Particle Helper\n component will do the setup for you",
                            smallLabel);

                DrawEffectSubKeywordToggle("Use grayscale as alpha? (Good for Additive configurations)",
                    "ALPHAFADEUSEREDCHANNEL_ON");
    ...
    void Blur(string inspector, string keyword, int first, int last)
    {
        if (SpecialCaseEffectBodyToggle(inspector, keyword))
        {
            EditorGUILayout.BeginVertical(propertiesStyle);
    ...
    void OffsetVertexStream(string inspector, string keyword, int first)
    {
        if (SpecialCaseEffectBodyToggle(inspector, keyword))
        {
            EditorGUILayout.BeginVertical(propertiesStyle);
    ...
    void ShapeWeightVertexStream(string inspector, string keyword, int first)
    {
        if (SpecialCaseEffectBodyToggle(inspector, keyword))
        {
            EditorGUILayout.BeginVertical(propertiesStyle);
    ...
    void ShapeTextureOffset(string inspector, string keyword, int first)
    {
        if (SpecialCaseEffectBodyToggle(inspector, keyword))
        {
            EditorGUILayout.BeginVertical(propertiesStyle);
    ...
    void ScreenDistort(string inspector, string keyword, int first, int last)
    {
        if (SpecialCaseEffectBodyToggle(inspector, keyword))
        {
            EditorGUILayout.BeginVertical(propertiesStyle);
    ...
    void FadeDissolve(string inspector, string keyword, int first, int last)
    {
        if (SpecialCaseEffectBodyToggle(inspector, keyword))
        {
            EditorGUILayout.BeginVertical(propertiesStyle);
    ...
    void PolarUvs(string inspector, string keyword)
    {
    ...
    void TrailWidth(string inspector, string keyword, int powerProperty, int gradientProperty)
    {
        if (SpecialCaseEffectBodyToggle(inspector, keyword))
        {
            EditorGUILayout.BeginVertical(propertiesStyle);
    ...
    void FakeLightAndShadow(string inspector, string keyword, int first, int last)
    {
        if (SpecialCaseEffectBodyToggle(inspector, keyword))
        {
            EditorGUILayout.BeginVertical(propertiesStyle);
    ...
    bool DrawEffectSubKeywordToggle(string inspector, string keyword, bool setCustomConfigAfter = false)
    {
    ...
    bool SpecialCaseEffectBodyToggle(string inspector, string keyword, string extraKeywordToDisable = null)
    {
        bool toggle = oldKeyWords.Contains(keyword);
        bool ini = toggle;
    ...
    void SetShaderBasedOnEffectsAndPipeline()
    {
        oldKeyWords = targetMat.shaderKeywords;
        ...
        if (pipeline.Equals("Built-In"))
        {
            if (oldKeyWords.Contains("SCREENDISTORTION_ON")) targetShader = "AllIn1VfxGrabPass";
                     ...
                     oldKeyWords.Contains("SHAPE2SCREENUV_ON") ||
                     oldKeyWords.Contains("SHAPE3SCREENUV_ON") || oldKeyWords.Contains("SOFTPART_ON") ||
                     oldKeyWords.Contains("DEPTHGLOW_ON")) targetShader = "AllIn1VfxBuiltIn";
        }
    ...
    bool SpecialCaseEffectHeaderToggle(string inspector, string keyword, bool toggle)
    {
        GUIContent effectNameLabel = new GUIContent();
        effectNameLabel.tooltip = keyword + " (C#)";
        effectNameLabel.text = effectCount + "." + inspector;
    ...
    void DrawProperty(int index, bool noReset = false)
    {
    ...
    void ResetProperty(MaterialProperty targetProperty)
    {
    ...
    void DrawMinMaxSlider(string sliderText, int propMin, int propMax, float valueMin = 0f, float valueMax = 1f)
    {
    ...
    static void SetLabelAndFieldWidth(int labelWidth, int fieldWidth)
    {
    ...
    void SetLabelWidthFromText(string text)
    {
    ...
    void SetFieldWidthFromText(string text)
    {
        Vector2 textDimensions = GUI.skin.label.CalcSize(new GUIContent(text));
    ...
    void DrawLine(Color color, int thickness = 2, int padding = 10)
    {

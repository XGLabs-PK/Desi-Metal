...
namespace AllIn1VfxToolkit
{
    ...
    public class AllIn1ParticleHelperComponent : MonoBehaviour
    {
...
#if UNITY_EDITOR
        float hueDif, saturationDif, targetHue, targetSaturation;
        ParticleSystem ps;
        ...
        public void HueShiftParticleSystem(Color tColor, int gradientColorKeyIndex = 0)
        {
            ...
            if (ps == null)
            {
                Debug.LogError("This GameObject: " + gameObject.name +
                               " doesn't have a Particle System and it should, please double check what you are doing");
            ...
            switch (mainModule.startColor.mode)
            {
                case ParticleSystemGradientMode.Color:
                    mainModule.startColor =
                        new ParticleSystem.MinMaxGradient(ShiftColorAndSaveDif(mainModule.startColor.color));

                    break;

                case ParticleSystemGradientMode.Gradient:
                case ParticleSystemGradientMode.RandomColor:
                    Gradient shiftedGradient =
                        GradientShiftAndSaveDif(mainModule.startColor.gradient, true, gradientColorKeyIndex);
                ...
                case ParticleSystemGradientMode.TwoColors:
                    mainModule.startColor = new ParticleSystem.MinMaxGradient(
                        ShiftColorAndSaveDif(mainModule.startColor.colorMax),
                ...
                switch (colorOverLifetimeModule.color.mode)
                {
                    case ParticleSystemGradientMode.Color:
                        colorOverLifetimeModule.color =
                            new ParticleSystem.MinMaxGradient(
                                ShiftColorAndSaveDif(colorOverLifetimeModule.color.color));

                        break;

                    case ParticleSystemGradientMode.Gradient:
                    case ParticleSystemGradientMode.RandomColor:
                        Gradient shiftedGradient = GradientShiftAndSaveDif(colorOverLifetimeModule.color.gradient, true,
                            gradientColorKeyIndex);
                    ...
                    case ParticleSystemGradientMode.TwoColors:
                        colorOverLifetimeModule.color = new ParticleSystem.MinMaxGradient(
                            ShiftColorAndSaveDif(colorOverLifetimeModule.color.colorMax),
                            ShiftColorFromPreCalculatedDif(colorOverLifetimeModule.color.colorMin));

                        break;

                    case ParticleSystemGradientMode.TwoGradients:
                        Gradient shiftedGradientMax =
                            GradientShiftAndSaveDif(colorOverLifetimeModule.color.gradientMax, true, 0);

                        Gradient shiftedGradientMin =
                            GradientShiftFromPreCalculatedDif(colorOverLifetimeModule.color.gradientMin);

                        colorOverLifetimeModule.color =
                            new ParticleSystem.MinMaxGradient(shiftedGradientMax, shiftedGradientMin);
                ...
                switch (colorBySpeedModule.color.mode)
                {
                    case ParticleSystemGradientMode.Color:
                        colorBySpeedModule.color =
                            new ParticleSystem.MinMaxGradient(ShiftColorAndSaveDif(colorBySpeedModule.color.color));

                        break;

                    case ParticleSystemGradientMode.Gradient:
                    case ParticleSystemGradientMode.RandomColor:
                        Gradient shiftedGradient = GradientShiftAndSaveDif(colorBySpeedModule.color.gradient, true,
                            gradientColorKeyIndex);
                    ...
                    case ParticleSystemGradientMode.TwoColors:
                        colorBySpeedModule.color = new ParticleSystem.MinMaxGradient(
                            ShiftColorAndSaveDif(colorBySpeedModule.color.colorMax),
                    ...
                    case ParticleSystemGradientMode.TwoGradients:
                        Gradient shiftedGradientMax =
                            GradientShiftAndSaveDif(colorBySpeedModule.color.gradientMax, true, 0);

                        Gradient shiftedGradientMin =
                            GradientShiftFromPreCalculatedDif(colorBySpeedModule.color.gradientMin);
        ...
        void CacheParticleSystem()
        {
            if (ps != null) return;
            ps = GetComponent<ParticleSystem>();
        }

        Gradient GradientShiftAndSaveDif(Gradient inGradient, bool saveColorOfFirstKey = false,
            int indexOfReferenceColor = 0)
        {
            var colorKeys = new GradientColorKey[inGradient.colorKeys.Length];
            var alphaKeys = new GradientAlphaKey[inGradient.alphaKeys.Length];
        ...
        Gradient GradientShiftFromPreCalculatedDif(Gradient inGradient)
        {
            var colorKeys = new GradientColorKey[inGradient.colorKeys.Length];
            var alphaKeys = new GradientAlphaKey[inGradient.alphaKeys.Length];
        ...
        Color ShiftColorAndSaveDif(Color colorToShift)
        {
            ...
            return Color.HSVToRGB(Mathf.Clamp01(colorHue + hueDif), Mathf.Clamp01(colorSaturation + saturationDif),
                colorValue);
        }

        Color ShiftColorFromPreCalculatedDif(Color colorToShift)
        {
            Color.RGBToHSV(colorToShift, out float colorHue, out float colorSaturation, out float colorValue);
        ...
        public void ApplyCurrentSettings(bool applyAllModules = false)
        {
            ...
            if (generalOptions || applyAllModules)
            {
                ...
                {
                    if (mainData.startLifetime.mode == ParticleSystemCurveMode.Curve)
                        mainData.duration = mainData.startLifetime.curveMax.Evaluate(1f);
                    else if (mainData.startLifetime.mode == ParticleSystemCurveMode.TwoCurves)
                        mainData.duration = Mathf.Max(mainData.startLifetime.curveMin.Evaluate(1f),
                            mainData.startLifetime.curveMax.Evaluate(1f));
                    else if (mainData.startLifetime.mode == ParticleSystemCurveMode.TwoConstants)
                        mainData.duration = Mathf.Max(mainData.startLifetime.constantMin,
                            mainData.startLifetime.constantMax);
            ...
            if (emissionOptions || applyAllModules)
            {
                ...
                if (isBurst)
                {
                    ...
                    if (sameMinAndMax)
                        emissionData.SetBursts(new ParticleSystem.Burst[]
                            { new ParticleSystem.Burst(0f, maxNumberOfParticles) });
                    else
                        emissionData.SetBursts(new ParticleSystem.Burst[]
                        {
                            new ParticleSystem.Burst(0f,
                                new ParticleSystem.MinMaxCurve(minNumberOfParticles, maxNumberOfParticles))
                        });
                }
                else
                {
                ...
                switch (colorLifetime)
                {
                    case LifetimeSettings.Ascendant:
                        newGradient.SetKeys(
                            ...
                            {
                                new GradientAlphaKey(0f, 0f), new GradientAlphaKey(0.65f, 0.5f),
                    ...
                    case LifetimeSettings.Descendent:
                        newGradient.SetKeys(
                            ...
                            new GradientAlphaKey[]
                            {
                                new GradientAlphaKey(1f, 0f), new GradientAlphaKey(1f, 0.25f),
        ...
        public void FetchGeneralValues()
        {
            ...
            if (mainData.startLifetime.mode == ParticleSystemCurveMode.Constant)
                minLifetime = mainData.startLifetime.constantMax;
            else minLifetime = mainData.startLifetime.constantMin;

            maxLifetime = mainData.startLifetime.constantMax;

            if (mainData.startSpeed.mode == ParticleSystemCurveMode.Constant)
                minSpeed = mainData.startSpeed.constantMax;
        ...
        public void FetchEmissionValues()
        {
            ...
            {
                if (emissionData.rateOverTime.mode == ParticleSystemCurveMode.Constant)
                    minNumberOfParticles = (int)emissionData.rateOverTime.constantMax;
        ...
        public void ApplyParticleHelperPreset(AllIn1ParticleHelperSO particleHelpersPreset,
            bool applyAllModules = false)
        {
            numberOfCopies = 1;
        ...
        public void SaveParticleHelperPreset()
        {
            ...
            if (PlayerPrefs.HasKey("All1VfxParticlePresets"))
                path = PlayerPrefs.GetString("All1VfxParticlePresets") + "/";
            ...
            if (System.IO.File.Exists(fullPath))
                fullPath = GetNewValidPath(path + gameObject.name + "HelperPreset", ".asset");
        ...
        public void SaveParticleSystemPreset()
        {
            ...
            if (System.IO.File.Exists(fullPath))
                fullPath = GetNewValidPath(path + gameObject.name + "PsPreset", ".preset");
        ...
        string GetNewValidPath(string path, string extension, int i = 1)
        {
            int number = i;
        ...
        IEnumerator DeleteAllChildHelperComponentsCR()
        {
            yield return null;
            var helpers = gameObject.GetComponentsInChildren<AllIn1ParticleHelperComponent>();
        ...
        public void CustomDataAutoSetup()
        {
            ...
            {
                Debug.LogError(
                    "The Particle System in the object: " + gameObject.name + " has no valid target material");

                return;
            }

            bool alphaEnable = targetMaterial.IsKeywordEnabled("ALPHAFADEINPUTSTREAM_ON") &&
                               (targetMaterial.IsKeywordEnabled("FADE_ON") ||
                                targetMaterial.IsKeywordEnabled("ALPHAFADE_ON"));
            ...
            if (alphaEnable)
            {
                ...
                customData.SetVector(ParticleSystemCustomData.Custom1, 1,
                    new ParticleSystem.MinMaxCurve(1, alphaFadeCurve));
            }
            ...
            if (texOffsetEnable)
            {
                ...
                customData.SetVector(ParticleSystemCustomData.Custom2, 0,
                    new ParticleSystem.MinMaxCurve(1, textureOffsetCurve));

                customData.SetVector(ParticleSystemCustomData.Custom2, 1,
                    new ParticleSystem.MinMaxCurve(1, textureOffsetCurve));
            }
            ...
            if (shapetWeightsEnable)
            {
                ...
                customData.SetVector(ParticleSystemCustomData.Custom2, 2,
                    new ParticleSystem.MinMaxCurve(1, weightOffsetCurve));
            }

            if (!texOffsetEnable && !shapetWeightsEnable)
            ...
            if (alphaEnable || texOffsetEnable || shapetWeightsEnable)
                rendererStreams.Add(ParticleSystemVertexStream.Custom1XY);

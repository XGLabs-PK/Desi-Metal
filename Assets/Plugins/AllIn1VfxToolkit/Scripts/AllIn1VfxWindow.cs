...
namespace AllIn1VfxToolkit
{
    public class AllIn1VfxWindow : EditorWindow
    {
        ...
        public static readonly string normalMapSavesPath =
            "Assets/AllIn1VfxToolkit/Demo & Assets/Textures/Distortion Normal Maps";
        public static readonly string gradientSavesPath =
            "Assets/AllIn1VfxToolkit/Demo & Assets/Textures/Color Gradients";
        ...
        public static readonly string materialAutoSetupSavesPath =
            "Assets/AllIn1VfxToolkit/Demo & Assets/Demo/Materials";

        const string Version = "1.3";
        public Vector2 scrollPosition = Vector2.zero;

        DefaultAsset materialTargetFolder = null;
        GUIStyle style, bigLabel = new GUIStyle();
        ...
        TextureSizes gradientSizes = TextureSizes._128;
        [SerializeField]
        Gradient gradient = new Gradient();
        FilterMode gradientFiltering = FilterMode.Bilinear;

        TextureSizes atlasSizesX = TextureSizes._512;
        TextureSizes atlasSizesY = TextureSizes._512;
        FilterMode atlasFiltering = FilterMode.Bilinear;
        ...
        Texture2D editorTex, editorTexInput, cleanEditorTex;
        ...
        void OnGUI()
        {
                       ...
                       GUILayout.Width(position.width), GUILayout.Height(position.height)))
            {
                ...
                currTab = GUILayout.Toolbar(currTab,
                    new string[] { "Save Paths", "Texture Editor", "Texture Creators", "Other" });
        ...
        void SavePaths()
        {
            ...
            GUILayout.Label(
                "Select the folder where new Materials will be saved when the Save Material To Folder button of the asset component is pressed",
            ...
            GUILayout.Label(
                "Select the folder where new Particle Helper Preset or Particle System Preset is saved with the Particle Helper Component",
                EditorStyles.boldLabel);
...

            {
                ...
                if (GUILayout.Button("Default Value", GUILayout.MaxWidth(100)))
                    PlayerPrefs.SetFloat("All1VfxRenderImagesScale", 1f);
            ...
            GUILayout.Label(
                "Select the folder where new Images will be saved when the Render Material To Image button of the asset component is pressed",
                EditorStyles.boldLabel);

            HandleSaveFolderEditorPref("All1VfxRenderImages", renderImagesSavesPath, "Images");
        }

        void HandleSaveFolderEditorPref(string keyName, string defaultPath, string logsFeatureName)
        {
            if (!PlayerPrefs.HasKey(keyName)) PlayerPrefs.SetString(keyName, defaultPath);

            materialTargetFolder =
                (DefaultAsset)AssetDatabase.LoadAssetAtPath(PlayerPrefs.GetString(keyName), typeof(DefaultAsset));
            ...
            materialTargetFolder = (DefaultAsset)EditorGUILayout.ObjectField("New " + logsFeatureName + " Folder",
                materialTargetFolder, typeof(DefaultAsset), false);
        ...
        void NormalMapCreator()
        {
            ...
            GUILayout.Label(
                "Select the folder where new Normal Maps will be saved when the Create Normal Map button of the asset component is pressed",
                EditorStyles.boldLabel);

            GUILayout.Label("*These Normal Maps can then be used with the Screen Distortion effect",
                EditorStyles.boldLabel);
            ...
            targetNormalImage = (Texture2D)EditorGUILayout.ObjectField("Target Image", targetNormalImage,
                typeof(Texture2D), false, GUILayout.MaxWidth(225));
            ...
            {
                GUILayout.Label("Normal Map is currently being created, be patient", EditorStyles.boldLabel,
                    GUILayout.Height(40));
                ...
                if (isComputingNormals > 5)
                {
                    ...
                    if (File.Exists(path)) path = GetNewValidPath(path);
            ...
            GUILayout.Label("*This process will freeze the editor for some seconds, larger images will take longer",
                EditorStyles.boldLabel);
        }

        static void SetTextureReadWrite(string assetPath, bool enable)
        {
        ...
        void GradientCreator()
        {
            GUILayout.Label("Color Gradient Creator", bigLabel);
            GUILayout.Space(20);

            GUILayout.Label("This feature can be used to create textures for the Color Ramp Effect",
                EditorStyles.boldLabel);
            ...
            GUILayout.Label("Select the folder where new Color Gradient Textures will be saved",
                EditorStyles.boldLabel);
        ...
        int atlasXCount = 1;
        int atlasYCount = 1;
        bool squareAtlas = true;

        void TextureAtlasPacker()
        {
...

            {
                if (squareAtlas)
                {
                    atlasXCount = EditorGUILayout.IntSlider("Column and Row Count", atlasXCount, 1, 8,
                        GUILayout.MaxWidth(302));
            ...
            GUILayout.Label("Output will be a " + atlasXCount + " X " + atlasYCount + " atlas, " + atlasElements +
                            " elements in total. In a " +
            ...
            if (usedAtlasSlots > atlasElements)
                GUILayout.Label(
                    "*Please reduce the Atlas texture slots by " + Mathf.Abs(atlasElements - Atlas.Length) +
                    " (extra textures will be ignored)", EditorStyles.boldLabel);

            if (atlasElements > usedAtlasSlots)
                GUILayout.Label(
                    "*" + (atlasElements - usedAtlasSlots) +
                    " atlas slots unused or null (it will be filled with black)", EditorStyles.boldLabel);
            ...
            if (Directory.Exists(prefSavedPath))
                if (GUILayout.Button("Create And Save Atlas Texture"))
                {
                    ...
                    if (path.Length != 0)
                    {
                        var AtlasCopy = (Texture2D[])Atlas.Clone();
                        ...
                        for (int j = 0; j < atlasXCount; j++)
                        {
                            ...
                            if (hasImageForThisIndex)
                            {
                                ...
                                Texture2D copyTexture = new Texture2D(AtlasCopy[currIndex].width,
                                    AtlasCopy[currIndex].height);

                                copyTexture.SetPixels(AtlasCopy[currIndex].GetPixels());
                                copyTexture.Apply();
                                AtlasCopy[currIndex] = copyTexture;

                                AtlasCopy[currIndex] = ScaleTexture(AtlasCopy[currIndex], textureXTargetWidth,
                                    textureYTargetHeight);
                            ...
                            for (int x = 0; x < textureXTargetWidth; x++)
                                if (hasImageForThisIndex)
                                    newAtlas.SetPixel(j * textureXTargetWidth + x, i * textureYTargetHeight + y,
                                        AtlasCopy[currIndex].GetPixel(x, y));
                                else
                                    newAtlas.SetPixel(j * textureXTargetWidth + x, i * textureYTargetHeight + y,
                                        new Color(0, 0, 0, 1));
                        }
        ...
        Texture2D noisePreview = null;
        RenderTexture noiseRenderTarget = null;
        Material noiseMaterial;
        float noiseScaleX = 10f, noiseScaleY = 10f, noiseContrast = 1f, noiseBrightness = 0f;
        float noiseFractalAmount = 1f, noiseJitter = 1f;
        int noiseSeed = 0;
        bool noiseSquareScale = false, noiseInverted = false, isFractalNoise;

        void CheckCreationNoiseTextures()
        {
            if (noisePreview == null) noisePreview = new Texture2D(256, 256);

            if (noiseRenderTarget == null)
                noiseRenderTarget =
                    new RenderTexture(noisePreview.width, noisePreview.height, 0, RenderTextureFormat.ARGB32);
        }

        void NoiseSetMaterial()
        {
        ...
        TextureSizes noiseSize = TextureSizes._512;
        FilterMode noiseFiltering = FilterMode.Bilinear;
        ...
        NoiseTypes noiseType = NoiseTypes.Fractal;
        ...
        void NoiseCreator()
        {
...

            {
...

                {
                    ...
                    noiseSquareScale =
                        EditorGUILayout.Toggle("Square Scale?", noiseSquareScale, GUILayout.MaxWidth(200));

                    if (noiseSquareScale) noiseScaleY = noiseScaleX;

                    if (noiseType == NoiseTypes.Fractal)
                        TextureEditorFloatParameter("Fractal Amount", ref noiseFractalAmount, 1f, 10f, 8f);
                    else if (noiseType == NoiseTypes.Perlin)
                        TextureEditorFloatParameter("Fractal Amount", ref noiseFractalAmount, 1f, 10f, 1f);
                    else if (noiseType == NoiseTypes.Billow)
                        TextureEditorFloatParameter("Fractal Amount", ref noiseFractalAmount, 1f, 10f, 4f);
            ...
            if (Directory.Exists(prefSavedPath) && noisePreview != null)
                if (GUILayout.Button("Save Noise Texture"))
                {
                    ...
                    if (path.Length != 0)
                    {
                        ...
                        RenderTexture finalRenderTarget = new RenderTexture(finalNoiseTex.width, finalNoiseTex.height,
                            0, RenderTextureFormat.ARGB32);

                        Graphics.Blit(finalNoiseTex, finalRenderTarget, noiseMaterial);

                        finalNoiseTex.ReadPixels(new Rect(0, 0, finalRenderTarget.width, finalRenderTarget.height), 0,
                            0);
        ...
        void OtherTab()
        {
            ...
            if (GUILayout.Button("Auto Setup Shaders for Materials in selected folder"))
            {
                ...
                string[] filePaths = Directory.GetFiles(autoSetupPath);

                if (filePaths != null && filePaths.Length > 0)
                    for (int i = 0; i < filePaths.Length; i++)
                    {
                        Object obj = AssetDatabase.LoadAssetAtPath(filePaths[i], typeof(Material));
        ...
        void SetShaderBasedOnEffectsAndPipeline(Material targetMat)
        {
        ...
        static bool IsAssetAFolder(Object obj)
        {
            string path = "";
        ...
        Texture2D ScaleTexture(Texture2D source, int targetWidth, int targetHeight)
        {
            ...
            var scaledPixels = result.GetPixels(0);
            float incX = (float)1 / source.width * ((float)source.width / targetWidth);
            float incY = (float)1 / source.height * ((float)source.height / targetHeight);

            for (int px = 0; px < scaledPixels.Length; px++)
                scaledPixels[px] = source.GetPixelBilinear(incX * ((float)px % targetWidth),
        ...
        Texture2D CreateNormalMap(Texture2D t, float normalMult = 5f, int normalSmooth = 0)
        {
            ...
            for (int x = 0; x < t.width; x++)
            {
                ...
                float edgeX = (fSampleNegXNegY - fSamplePosXNegY) * 0.25f + (fSampleNegXZerY - fSamplePosXZerY) * 0.5f +
                              (fSampleNegXPosY - fSamplePosXPosY) * 0.25f;

                float edgeY = (fSampleNegXNegY - fSampleNegXPosY) * 0.25f + (fSampleZerXNegY - fSampleZerXPosY) * 0.5f +
        ...
        Color editorColorTint = Color.white;
        float brightness = 0f, contrast = 1f, gamma = 1f, exposure = 0f, saturation = 1f, hue = 0f;
        bool invert = false,
            greyscale = false,
            fullWhite = false,
        ...
        bool isFlipHorizontal = false, isFlipVertical = false;
        ...
        void TextureEditor()
        {
            ...
            editorTexInput = EditorGUILayout.ObjectField("Image to Edit", editorTexInput, typeof(Texture2D), false,
                GUILayout.Width(300), GUILayout.Height(50)) as Texture2D;
            ...
            if (editorTex != null)
            {
                ...
                GUILayout.Label(
                    "*Preview is locked to 256px maximum (bigger textures are scaled down), but the image will be saved to its full resolution",
                    EditorStyles.boldLabel);
            }
            else
            {
            ...
            GUILayout.Label(
                "Current export size is: " + currWidth + " x " + currHeight + " (size snaps to the closest power of 2)",
                EditorStyles.boldLabel);
        ...
        void ComputeFinalTexture()
        {
            ...
            if (Math.Abs(exportScale - 1f) > 0.05f)
                editorTexInput = ScaleTexture(editorTexInput, (int)(texWidth * exportScale),
                    (int)(texHeight * exportScale));
        }

        void SetTextureEditorDefaultValues()
        {
        ...
        void RecalculateEditorTexture()
        {
            var pixels = cleanEditorTex.GetPixels();
        ...
        void ComputeImageColorFilters(Color[] pixels)
        {
            ...
            for (int i = 0; i < pixels.Length; i++)
            {
                ...
                pixels[i] = new Color(Mathf.Clamp01(pixels[i].r * (1 + brightness)),
                    Mathf.Clamp01(pixels[i].g * (1 + brightness)), Mathf.Clamp01(pixels[i].b * (1 + brightness)),
                    pixels[i].a);
                ...
                hueShiftColor.r = Mathf.Clamp01((.299f + .701f * cosHsv + .168f * sinHsv) * pixels[i].r +
                                                (.587f - .587f * cosHsv + .330f * sinHsv) * pixels[i].g +
                                                (.114f - .114f * cosHsv - .497f * sinHsv) * pixels[i].b);

                hueShiftColor.g = Mathf.Clamp01((.299f - .299f * cosHsv - .328f * sinHsv) * pixels[i].r +
                                                (.587f + .413f * cosHsv + .035f * sinHsv) * pixels[i].g +
                                                (.114f - .114f * cosHsv + .292f * sinHsv) * pixels[i].b);

                hueShiftColor.b = Mathf.Clamp01((.299f - .3f * cosHsv + 1.25f * sinHsv) * pixels[i].r +
                                                (.587f - .588f * cosHsv - 1.05f * sinHsv) * pixels[i].g +
                                                (.114f + .886f * cosHsv - .203f * sinHsv) * pixels[i].b);
        ...
        void TextureEditorFloatParameter(string parameterName, ref float parameter, float rangeMin = -100f,
            float rangeMax = 100f, float resetValue = 0f)
        {
            EditorGUILayout.BeginHorizontal();

            {
                parameter = EditorGUILayout.Slider(parameterName, parameter, rangeMin, rangeMax,
        ...
        void TextureEditorIntParameter(string parameterName, ref int parameter, int rangeMin = -100, int rangeMax = 100,
            int resetValue = 0)
        {
            EditorGUILayout.BeginHorizontal();

            {
                parameter = EditorGUILayout.IntSlider(parameterName, parameter, rangeMin, rangeMax,
                    GUILayout.MaxWidth(400));
        ...
        void TextureEditorColorParameter(string parameterName, ref Color parameter, Color resetValue)
        {
            EditorGUILayout.BeginHorizontal();

            {
                ...
                parameter = EditorGUILayout.ColorField(colorLabel, parameter, true, true, true,
                    GUILayout.MaxWidth(400));
        ...
        void RotateEditorTextureLeft()
        {
            var pixels = editorTex.GetPixels();
            var pixelsClean = cleanEditorTex.GetPixels();
        ...
        Color[] RotateClockWise(Color[] pixels, int width, int height)
        {
        ...
        void FlipEditorTexture(bool isHorizontal)
        {
            var pixels = editorTex.GetPixels();
            var pixelsClean = cleanEditorTex.GetPixels();
            int texWidth = editorTex.width;
        ...
        Color[] FlipHorizontal(Color[] pixels, int width, int height)
        {
            var outputPixels = new Color[pixels.Length];
        ...
        Color[] FlipVertical(Color[] pixels, int width, int height)
        {
            var outputPixels = new Color[pixels.Length];

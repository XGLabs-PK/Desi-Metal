...
namespace MoreMountains.Feedbacks
{
    ...
    public class MMFeedbackMaterial : MMFeedback
    {
        ...
        {
            get => InterpolateTransition ? TransitionDuration : 0f;
            set
            {
        ...
        protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
        {
            ...
            {
                Debug.LogError("[MMFeedbackMaterial on " + name + "] The Materials array is empty.");
                return;
            }
            ...
            {
                Debug.LogError("[MMFeedbackMaterial on " + name + "] Attempting to switch to a null material.");
                return;
            }

            if (InterpolateTransition)
                for (int i = 0; i < RendererMaterialIndexes.Length; i++)
                    _coroutines[i] = StartCoroutine(TransitionMaterial(

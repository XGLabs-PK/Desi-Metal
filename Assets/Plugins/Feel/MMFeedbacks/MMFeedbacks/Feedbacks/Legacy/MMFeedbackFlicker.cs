...
namespace MoreMountains.Feedbacks
{
    ...
    public class MMFeedbackFlicker : MMFeedback
    {
        ...
        protected override void CustomInitialization(GameObject owner)
        {
            ...
            if (BoundRenderer == null)
                Debug.LogWarning("[MMFeedbackFlicker] The flicker feedback on " + name +
                ...
                if (Active && BoundRenderer != null)
                {
                    if (Mode == Modes.Color)
                    {
                        _propertiesFound[i] = UseMaterialPropertyBlocks
                            ? BoundRenderer.sharedMaterials[i].HasProperty(_colorPropertyName)
                            : BoundRenderer.materials[i].HasProperty(_colorPropertyName);

                        if (_propertiesFound[i])
                            _initialFlickerColors[i] = UseMaterialPropertyBlocks
                                ? BoundRenderer.sharedMaterials[i].color
                                : BoundRenderer.materials[i].color;
                    }
                    else
                    {
                        _propertiesFound[i] = UseMaterialPropertyBlocks
                            ? BoundRenderer.sharedMaterials[i].HasProperty(PropertyName)
                            : BoundRenderer.materials[i].HasProperty(PropertyName);
                        ...
                        if (_propertiesFound[i])
                        {
                            ...
                            _initialFlickerColors[i] = UseMaterialPropertyBlocks
                                ? BoundRenderer.sharedMaterials[i].GetColor(_propertyIDs[i])
                                : BoundRenderer.materials[i].GetColor(_propertyIDs[i]);
                        }
                    }
                }
            }

...
namespace MoreMountains.Feedbacks
{
    ...
    public class MMFeedbackParticlesInstantiation : MMFeedback
    {
        ...
        protected virtual void InstantiateParticleSystem()
        {
            ...
            if (RandomParticlePrefabs.Count > 0)
            {
                if (Mode == Modes.Cached)
                {
                    ...
                    {
                        ParticleSystem newSystem = Instantiate(system, newParent) as ParticleSystem;

                        if (newParent == null)
                            SceneManager.MoveGameObjectToScene(newSystem.gameObject, gameObject.scene);
                ...
                else
                {
                    ...
                    _instantiatedParticleSystem =
                        Instantiate(RandomParticlePrefabs[random], newParent) as ParticleSystem;

                    if (newParent == null)
                        SceneManager.MoveGameObjectToScene(_instantiatedParticleSystem.gameObject, gameObject.scene);
                }
            }
            else
            {
                _instantiatedParticleSystem = Instantiate(ParticlesPrefab, newParent) as ParticleSystem;
        ...
        protected virtual void PositionParticleSystem(ParticleSystem system)
        {
            ...
            system.transform.position = GetPosition(transform.position);

            if (ApplyRotation)
                system.transform.rotation = GetRotation(transform);

            if (ApplyScale)
                system.transform.localScale = GetScale(transform);

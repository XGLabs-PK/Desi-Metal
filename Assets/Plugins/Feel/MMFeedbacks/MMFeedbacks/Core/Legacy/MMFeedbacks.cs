    ...
    public class MMFeedbacks : MonoBehaviour
    {
        ...
        protected virtual void Awake()
        {
            ...
            if (AutoPlayOnEnable)
            {
                ...
                if (enabler == null)
                    enabler = gameObject.AddComponent<MMFeedbacksEnabler>();
        ...
        {
            PlayFeedbacksInternal(transform.position, FeedbacksIntensity);
        }
        ...
        {
            PlayFeedbacksInternal(transform.position, FeedbacksIntensity, true);
        }
        ...
        protected virtual IEnumerator PausedFeedbacksCo(Vector3 position, float feedbacksIntensity)
        {
            ...
            while (i >= 0 && i < Feedbacks.Count)
            {
                ...
                // updates holding max
                if (Feedbacks[i].Active)
                    if (Feedbacks[i].Pause == null && Feedbacks[i].ShouldPlayInThisSequenceDirection &&
        ...
        public virtual MMFeedback AddFeedback(Type feedbackType)
        {
            ...
            if (!Application.isPlaying)
                newFeedback = Undo.AddComponent(gameObject, feedbackType) as MMFeedback;
            else
                newFeedback = gameObject.AddComponent(feedbackType) as MMFeedback;
#else

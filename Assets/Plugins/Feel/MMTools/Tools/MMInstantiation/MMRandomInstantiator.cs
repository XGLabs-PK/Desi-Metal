...
namespace MoreMountains.Tools
{
    ...
    public class MMRandomInstantiator : MonoBehaviour
    {
        ...
        public virtual void InstantiateRandomObject()
        {
            ...
            _instantiatedGameObject = Instantiate(RandomPool[randomIndex], transform.position, transform.rotation);
            SceneManager.MoveGameObjectToScene(_instantiatedGameObject, gameObject.scene);
            _instantiatedGameObject.name = InstantiatedObjectName;

            if (ParentInstantiatedToThisObject)
                _instantiatedGameObject.transform.SetParent(transform);
        }
    }
}

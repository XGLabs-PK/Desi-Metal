...
namespace MoreMountains.Tools
{
    ...
    public class MMConeOfVision2D : MonoBehaviour
    {
        ...
        protected virtual void ScanForTargets()
        {
            ...
            _targetsWithinDistance = Physics2D.OverlapCircleAll(transform.position, VisionRadius, TargetMask);
            ...
            foreach (Collider2D collider in _targetsWithinDistance)
            {
                ...
                if (Vector3.Angle(Direction, _directionToTarget) < VisionAngle / 2f)
                {
                    ...
                    _scanForTargetsHit2D = Physics2D.Raycast(transform.position, _directionToTarget, _distanceToTarget,
                        ObstacleMask);
        ...
        RaycastData RaycastAtAngle(float angle)
        {
            ...
            _raycastAtAngleHit2D = Physics2D.Raycast(transform.position, _direction, VisionRadius, ObstacleMask);
            ...
            else
            {
                ...
                _returnRaycastData.Point = transform.position + _direction * VisionRadius;

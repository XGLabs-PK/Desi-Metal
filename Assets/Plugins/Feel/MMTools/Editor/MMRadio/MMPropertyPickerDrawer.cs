...
namespace MoreMountains.Tools
{
    ...
    public class MMPropertyPickerDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            Rect targetObjectRect = new Rect(position.x, position.y + (_lineHeight + _lineMargin), position.width,
                _lineHeight);

            Rect targetComponentRect = new Rect(position.x, position.y + (_lineHeight + _lineMargin) * 2,
            ...
            if (_componentNames != null && _componentNames.Length > 0)
            {
                ...
                _selectedComponentIndex = EditorGUI.Popup(targetComponentRect, "Component", _selectedComponentIndex,
                    _componentNames);
            ...
            if ((_selectedComponentIndex != 0 || _targetIsScriptableObject) && _propertiesNames != null &&
                _propertiesNames.Length > 0)
            {
                EditorGUI.BeginChangeCheck();

                _selectedPropertyIndex = EditorGUI.Popup(targetPropertyRect, "Property", _selectedPropertyIndex,
                    _propertiesNames);
        ...
        protected virtual void FillPropertyList(SerializedProperty property)
        {
            ...
            // we create a temp list to fill our array with
            var tempPropertiesList = new List<string>();
        ...
        protected virtual void SetTargetProperty(SerializedProperty property)
        {
            if (_selectedPropertyIndex > 0)
            {
                ...
                property.FindPropertyRelative("TargetPropertyName").stringValue =
                    _propertiesList[_selectedPropertyIndex];

...
namespace Pathfinding
{
    ...
    public abstract class NavmeshBase : NavGraph, INavmesh, INavmeshHolder, ITransformedGraph
    {
        ...
        public IntRect GetTouchingTiles(Bounds bounds, float margin = 0)
        {
            ...
            IntRect r = new IntRect(Mathf.FloorToInt((bounds.min.x - margin) / TileWorldSizeX),
        ...
        {
            // Calculate world bounds of all affected tiles
            IntRect r = new IntRect(Mathf.FloorToInt(rect.xMin / TileWorldSizeX),
        ...
        public IntRect GetTouchingTilesRound(Bounds bounds)
        {
            ...
            //Calculate world bounds of all affected tiles
            IntRect r = new IntRect(Mathf.RoundToInt(bounds.min.x / TileWorldSizeX),
        ...
        protected void ConnectTileWithNeighbours(NavmeshTile tile, bool onlyUnflagged = false)
        {
            ...
            for (int zo = -1; zo <= 1; zo++)
            {
                ...
                for (int xo = -1; xo <= 1; xo++)
                {
                    ...
                    NavmeshTile otherTile = tiles[x + z * tileXCount];
        ...
        public override NNInfoInternal GetNearestForce(Vector3 position, NNConstraint constraint)
        {
            ...
            Int2 tileCoords = GetTileCoordinates(position);
            ...
            NNInfoInternal best = new NNInfoInternal();
        ...
        protected static void CreateNodeConnections(TriangleMeshNode[] nodes)
        {
            ...
            for (int i = 0; i < nodes.Length; i++)
            {
                ...
                for (int a = 0; a < av; a++)
                {
                    ...
                    Int2 key = new Int2(node.GetVertexIndex(a), node.GetVertexIndex((a + 1) % av));
        ...
        void DestroyNodes(List<MeshNode> nodes)
        {
            ...
            for (int i = 0; i < batchNodesToDestroy.Count; i++)
            {
                MeshNode node = batchNodesToDestroy[i];

                for (int j = 0; j < node.connections.Length; j++)
                {
                    GraphNode neighbour = node.connections[j].node;
        ...
        void PrepareNodeRecycling(int x, int z, Int3[] verts, int[] tris, TriangleMeshNode[] recycledNodeBuffer)
        {
            ...
            for (int i = 0; i < nodes.Length; i++)
            {
                ...
                int hash = v0.GetHashCode() + v1.GetHashCode() + v2.GetHashCode();
                int newNodeIndex;
        ...
        public void ReplaceTile(int x, int z, Int3[] verts, int[] tris)
        {
            ...
            bool wasNotBatching = !batchTileUpdate;
            ...
            Int3 offset = (Int3)new Vector3(x * TileWorldSizeX, 0, z * TileWorldSizeZ);
        ...
        protected void CreateNodes(TriangleMeshNode[] buffer, int[] tris, int tileIndex, uint graphIndex)
        {
            ...
            {
                TriangleMeshNode node = buffer[i];
        ...
        public override void OnDrawGizmos(RetainedGizmos gizmos, bool drawNodes)
        {
            ...
            {
                RetainedGizmos.Hasher baseHasher = new RetainedGizmos.Hasher(active);
                ...
                RetainedGizmos.Hasher hasher = baseHasher;
                int hashedNodes = 0;
        ...
        void CreateNavmeshSurfaceVisualization(NavmeshTile[] tiles, int startTile, int endTile, GraphGizmoHelper helper)
        {
            ...
            for (int i = startTile; i < endTile; i++)
            {
                NavmeshTile tile = tiles[i];
                if (tile == null) continue;

                for (int j = 0; j < tile.nodes.Length; j++)
                {
                    TriangleMeshNode node = tile.nodes[j];
                    Int3 v0, v1, v2;
                    ...
                    Color color = helper.NodeColor(node);
            ...
            GraphGizmoHelper helper)
        {
            ...
            for (int i = startTile; i < endTile; i++)
            {
                ...
                for (int j = 0; j < tile.nodes.Length; j++)
                {
                    ...
                    {
                        TriangleMeshNode other = node.connections[c].node as TriangleMeshNode;
        ...
        protected override void DeserializeExtraInfo(GraphSerializationContext ctx)
        {
            ...
            for (int x = 0; x < tileXCount; x++)
            {
                ...
                NavmeshTile tile = tiles[tileIndex] = new NavmeshTile
                {
                    x = tx,
                    z = tz,
                    w = reader.ReadInt32(),
                ...
                if (trisCount % 3 != 0)
                    throw new System.Exception("Corrupt data. Triangle indices count must be divisable by 3. Read " +
                                               trisCount);
        ...
        protected override void PostDeserialization(GraphSerializationContext ctx)
        {
            ...
            if (ctx.meta.version < AstarSerializer.V4_1_0 && tiles != null)
            {
                ...
                foreach (NavmeshTile tile in tiles) CreateNodeConnections(tile.nodes);
                foreach (NavmeshTile tile in tiles) ConnectTileWithNeighbours(tile);
                ...
                GetNodes(node =>
                {
                    ...
                    foreach (Connection conn in conns[triNode].Where(conn => !triNode.ContainsConnection(conn.node))
                                 .ToList())

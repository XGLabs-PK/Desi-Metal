...
namespace Pathfinding
{
    ...
    public class GridGraph : NavGraph, IUpdatableGraph, ITransformedGraph
    {
        ...
        public GraphTransform CalculateTransform()
        {
            ...
            Matrix4x4 boundsMatrix = Matrix4x4.TRS(center, Quaternion.Euler(rotation), new Vector3(aspectRatio, 1, 1)) *
                                     isometricMatrix;
            ...
            Matrix4x4 m =
                Matrix4x4.TRS(
                    boundsMatrix.MultiplyPoint3x4(
                        -new Vector3(newWidth * newNodeSize, 0, newDepth * newNodeSize) * 0.5F),
                    Quaternion.Euler(rotation), new Vector3(newNodeSize * aspectRatio, 1, newNodeSize)) *
                isometricMatrix;
        ...
        void CreateNavmeshSurfaceVisualization(GridNodeBase[] nodes, int nodeCount, GraphGizmoHelper helper)
        {
            ...
            for (int i = 0; i < nodeCount; i++)
            {
                ...
                {
                    GridNodeBase other =
                        node.GetNeighbourAlongDirection(neighbourIndices[(j + 1) % neighbourIndices.Length]);

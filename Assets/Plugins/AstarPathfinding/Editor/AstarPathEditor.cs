...
namespace Pathfinding
{
    ...
    public class AstarPathEditor : Editor
    {
        ...
        Type[] graphTypes => script.data.graphTypes;
        ...
        static bool LoadStyles()
        {
            ...
            string skinPath = EditorResourceHelper.editorAssets + "/AstarEditorSkin" +
...
#if !ASTAR_ATAVISM
            Version newVersion = AstarUpdateChecker.latestVersion;
            bool beta = false;
        ...
        void DrawGraphHeader(GraphEditor graphEditor)
        {
            ...
            {
                bool moveUp = GUILayout.Button(new GUIContent("Up", "Increase the graph priority"),
                    GUILayout.Width(40));

                bool moveDown = GUILayout.Button(new GUIContent("Down", "Decrease the graph priority"),
                    GUILayout.Width(40));
        ...
        void CheckGraphEditors(bool forceRebuild = false)
        {
                ...
                script.graphs.Length != graphEditors.Length)
            {
                ...
                for (int i = 0; i < script.graphs.Length; i++)
                {
                    ...
                    if (graph.guid == new Util.Guid())
                        graph.guid = Util.Guid.NewGuid();
                ...
                for (int i = 0; i < script.graphs.Length; i++)
                {
                    ...
                    if (script.graphs[i].guid == new Util.Guid())
                        script.graphs[i].guid = Util.Guid.NewGuid();
        ...
        {
            SerializeSettings settings = SerializeSettings.Settings;
                ...
                foreach (Type type in types)
                {
                    ...
                    while (!System.Type.Equals(baseType, null))
                    {
                        if (System.Type.Equals(baseType, typeof(GraphEditor)))
                        {
                            ...
                            foreach (object attribute in att)
                            {
